(function() {
    ...
    // ============================================================
    // HOME / DASHBOARD (com cards de promessas e atrasos)
    // ============================================================
    async function renderHome() {
        ...
        try {
            ...
            // Processar promessas de pagamento
            let promessasHoje = [];
            let promessasAtraso = [];
            ...
            if (promessasResult.success && promessasResult.promessas) {
                promessasResult.promessas.forEach(p => {
                    ...
                    if (dataPromessaStr === hojeStr && saldo > 0) {
            ...
            // Montar HTML dos cards de promessas
    ...
    window.enviarCobranca = function(cliente, valor, whatsapp) {
        const mensagem = `Olá amiga, ${cliente}! 👋\n\n` +
                         `Lembrando do pagamento da sua conta no valor de R$ ${valor.toFixed(2).replace('.', ',')}.\n\n` +
        ...
        // Se tiver WhatsApp, tenta usar o número específico
        if (whatsapp) {
                ...
                <section>
                    ...
                    <div style="background:white; padding:30px; border-radius:12px; box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                        <form id="formVendaMultipla">
                            ...
                            <!-- NOVOS CAMPOS: WhatsApp e Data de Pagamento -->
    ...
    async function registrarVendaMultipla(e) {
        ...
        if (troco >= 0) {
            ...
            // Se não pagou na hora e tem data de pagamento, adiciona à mensagem
            if (dataPagamento) {
            ...
            async () => {
                try {
                    ...
                    if (todasOk) {
                        ...
                        // Registrar promessa de pagamento se houver saldo pendente E data de pagamento
    ...
    // ============================================================
    // ÁREA DA VENDEDORA (com promessas de pagamento)
    // ============================================================
    async function renderVendedora() {
        ...
        app.innerHTML = `
            <section>
                ...
                <div style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                    ...
                    <!-- Botões para extratos de vendas -->
                    <div style="margin-bottom: 20px;">
                    ...
                    <!-- Botões para extratos de pagamentos recebidos -->
        ...
        // Carregar promessas do dia
        await carregarPromessasHoje();
    }

    // ============================================================
    // CARREGAR PROMESSAS DE PAGAMENTO DO DIA
    // ============================================================
    ...
    // ============================================================
    // FUNÇÕES DE EXTRATO (mantidas do código anterior)
    // ============================================================
    // ... (manter todas as funções de extrato, compartilhamento, etc.)
    // Por questões de espaço, as funções abaixo são as mesmas do código anterior:
    // - gerarExtrato
    // - gerarExtratoPagamentos
    // - compartilharExtratoVendedora
    // - compartilharExtratoPagamentos
    // - imprimirExtrato
    // - fecharResultadoExtrato
    // - atualizarVendedora
