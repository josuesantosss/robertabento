// ============================================================
// SISTEMA DE VENDAS - VERSÃO OTIMIZADA V15.0
// ============================================================
...
(function() {
    ...
    // ============================================================
    const SISTEMA = {
        VERSAO: 'v15.0',        // ← Mude aqui para atualizar a versão
        DATA: '2026',
    ...
    function adicionarEstilosCSS() {
        ...
        style.textContent = `
            ...
            .busca-produto-box{transition:all 0.3s ease}
            .busca-produto-box:focus-within{box-shadow:0 0 0 3px rgba(102,126,234,0.2);border-color:#667eea !important}
            .produto-busca-item{transition:all 0.2s ease}
            .produto-busca-item:hover{background:#f7fafc !important;transform:translateX(4px)}
    ...
    const StateManager = {
        ...
        setFiltroMes(mes) { this.filtroMes = mes; },
        getFiltroMes() { return this.filtroMes; }
    };
    ...
    // ============================================================
    // ESTOQUE OTIMIZADO (COM BUSCA DE PRODUTOS)
    // ============================================================
        ...
        try {
            ...
            app.innerHTML = `
                <section style="animation:fadeIn 0.4s ease;">
                    ...
                    <!-- 🔍 NOVA BOX DE BUSCA DE PRODUTOS -->
                    <div style="background:#fff;padding:15px;border-radius:12px;margin-bottom:15px;box-shadow:0 2px 8px rgba(0,0,0,0.1);border:2px solid #e2e8f0;" class="busca-produto-box">
                        <h3 style="margin:0 0 10px 0;color:#4a5568;font-size:15px;">🔍 Buscar Produtos</h3>
                        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                            <div style="flex:1;min-width:200px;position:relative;">
                                <input type="text" id="buscaProduto" placeholder="Digite o nome do produto para filtrar..." 
                                       style="width:100%;padding:10px 36px 10px 12px;border:2px solid #e2e8f0;border-radius:8px;font-size:14px;outline:none;transition:all 0.3s ease;">
                                <span style="position:absolute;right:10px;top:50%;transform:translateY(-50%);font-size:16px;color:#a0aec0;pointer-events:none;">🔎</span>
                            </div>
                            <button type="button" id="btnLimparBuscaProduto" 
                                    style="background:#e2e8f0;color:#4a5568;border:none;padding:10px 18px;border-radius:8px;cursor:pointer;font-weight:500;font-size:13px;transition:all 0.2s ease;">
                                ✕ Limpar
                            </button>
                        </div>
                        <div id="resultadoBuscaProduto" style="margin-top:10px;font-size:13px;color:#4a5568;min-height:20px;"></div>
                    </div>
                    <!-- FIM DA BOX DE BUSCA -->
                    ...
                    <div style="background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);overflow:hidden;">
                        <div style="overflow-x:auto;">
                            <table style="width:100%;border-collapse:collapse;font-size:13px;">
                                ...
                                <tbody id="tabelaProdutosBody">${html}</tbody>
                            </table>
                        </div>
                    </div>
                </section>
            ...
            const btnLimpar = document.getElementById('btnLimparBuscaProduto');
            const tbody = document.getElementById('tabelaProdutosBody');

            // Guarda o HTML original da tabela para restaurar
            const htmlOriginalTabela = tbody.innerHTML;

            function filtrarProdutos() {
                const termo = inputBusca.value.trim().toLowerCase();
                if (!termo) {
                    tbody.innerHTML = htmlOriginalTabela;
                ...
                if (encontrados === 0) {
                    tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;padding:30px;"><p style="font-size:32px;">🔍</p><p style="color:#666;">Nenhum produto encontrado para "${inputBusca.value.trim()}"</p></td></tr>`;
                    resultadoBusca.innerHTML = `<span style="color:#e53e3e;">❌ Nenhum produto encontrado</span>`;
                } else {
                    resultadoBusca.innerHTML = `<span style="color:#48bb78;">✅ ${encontrados} produto(s) encontrado(s)</span>`;
                }
            }

            const filtrarDebounced = debounce(filtrarProdutos, 250);
            inputBusca.addEventListener('input', filtrarDebounced);
    ...
    // ============================================================
    // FUNÇÃO GLOBAL PADRÃO (substitui mostrarDetalhesCliente)
    // ============================================================
    window.mostrarDetalhesCliente = function(nome) {
        StateManager.setClienteExpandido(nome);
        carregarTabelaClientes(document.getElementById('buscaCliente')?.value || '');
    };
    ...
    function init() {
        ...
        const page = StateManager.getPage() || 'home';
        const pageMap = {
            'home': renderHome,
            'estoque': renderEstoque,
            'vendas': renderVendas,
    ...
    window.registrarPagamentoClienteDetalhe = window.registrarPagamentoInline;
    window.compartilharExtrato = window.compartilharExtrato;
    window.aplicarFiltroMesPagamentos = window.aplicarFiltroMesPagamentos;
    window.atualizarVendedora = window.renderVendedora;
    ...
    console.log('🚀 Sistema de Vendas v4.0 – Estoque com cadastro rápido, edição/exclusão no modal, extrato WhatsApp e filtro de pagamentos por mês');
})();
